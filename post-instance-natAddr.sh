...
for instance_name in "${instance_names[@]}"; do
  ...
  json_payload=$(cat "$DEST_DIR/instance_${instance_name}_natAddress_details.json")
  ...
  echo "$create_response" > "$DEST_DIR/instance_${instance_name}_natAddress_response.json"

  echo "Environment $instance_name nataddress created in the destination project."
done