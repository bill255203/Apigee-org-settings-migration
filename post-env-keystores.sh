...
for environment in "${environments[@]}"; do
  ...
  for keystore in "${keystore_names[@]}"; do
    ...
    json_payload=$(cat "$DEST_DIR/${environment}_env_${keystore}_keystore_details.json")

    # Make a POST request to create the environment in the destination project
    create_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/environments/$environment/keystores" \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        -H "Content-Type: application/json")

    # Save the response for the created environment to a file
    echo "$create_response" > "$DEST_DIR/${environment}_env_${keystore}_keystore_response.json"
