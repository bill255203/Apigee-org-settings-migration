...
#     url="https://apigee.googleapis.com/v1/organizations/$SOURCE_ORG/apis/$name/revisions/$revision/debugsessions"

#     # Perform the individual curl request to download the ZIP bundle to the specified directory
#     curl -X GET "$url" -H "Authorization: Bearer $SOURCE_TOKEN" -o "$DEST_DIR/proxy_${name}_revision_${revision}_debugsessions.json"
#   done
# done
