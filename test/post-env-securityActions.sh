...
for environment in "${environments[@]}"; do
  securityAction_names=($(cat "$DEST_DIR/${environment}_securityActions_details.json" | jq -r '.[]'))

  for securityAction in "${securityAction_names[@]}"; do
    # Create the JSON payload using data from the environment details file
    json_payload=$(cat "$DEST_DIR/${environment}_env_${securityAction}_securityAction_details.json")

    # Make a POST request to create the environment in the destination project
    create_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/environments/$environment/securityActions" \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        -H "Content-Type: application/json")

    # Save the response for the created environment to a file
    echo "$create_response" > "$DEST_DIR/${environment}_env_${securityAction}_securityAction_response.json"
