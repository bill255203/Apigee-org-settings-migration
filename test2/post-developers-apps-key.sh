...
for email in "${emails[@]}"; do
  ...
  for appId in "${appIds[@]}"; do
    ...
    for key in "${keys[@]}"; do
        json_payload=$(cat "$DEST_DIR/${email}_${appId}_dev_app_${key}_keys.json")
        echo "$json_payload"
        # Make the API call to get detailed information for the app and save it to a file
        post_dev_app_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$SOURCE_ORG/developers/$email/apps/$appId/attributes/$key"  \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        --header 'Accept: application/json' \
        -H "Content-Type: application/json")
        # Save the response for the app details to a file
        echo "$post_dev_app_response" > "$DEST_DIR/${email}_${appId}_dev_app_${key}_key_response.json"
        echo "App details saved to $DEST_DIR/${email}_${appId}_dev_app_${key}_key_response.json"
  done
