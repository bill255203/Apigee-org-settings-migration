  ...
  # Parse the app IDs from the app details JSON file using jq
    ...
    for name in "${names[@]}"; do
      ...
      for key in "${keys[@]}"; do
        ...
        json_payload=$(cat "$DEST_DIR/${appgroup_name}_${name}_app_${key}_key_details.json")
        # Echo the JSON payload before making the POST request
        echo "JSON Payload:"
        echo "$json_payload"
        # Make a GET request to get app details
        post_app_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/appgroups/${appgroup_name}/apps/${name}/keys" \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        -H "Content-Type: application/json")
        # Save the response for the app details to a file
        echo "$post_app_response" > "$DEST_DIR/${appgroup_name}_${name}_app_${key}_key_response.json"

        echo "App details saved to $DEST_DIR/${appgroup_name}_${name}_app_${key}_key_response.json"
      done
  done
done
