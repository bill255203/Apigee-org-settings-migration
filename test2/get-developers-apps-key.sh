...
for email in "${emails[@]}"; do
  ...
  for appId in "${appIds[@]}"; do
    ...
    curl -X GET "https://apigee.googleapis.com/v1/organizations/$SOURCE_ORG/developers/$email/apps/$transformed_appId/keys" -H "Authorization: Bearer $SOURCE_TOKEN" -o "$DEST_DIR/${email}_${appId}_dev_app_keys.json"
    
    keys=($(jq -r '.attribute[].name' "$DEST_DIR/${email}_${appId}_dev_app_keys.json"))

    for key in "${keys[@]}"; do
      curl -X GET "https://apigee.googleapis.com/v1/organizations/$SOURCE_ORG/developers/$email/apps/$transformed_appId/keys/$key" -H "Authorization: Bearer $SOURCE_TOKEN" -o "$DEST_DIR/${email}_${appId}_dev_app_${key}_key_details.json"
    done
  done
done
