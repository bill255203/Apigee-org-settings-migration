...
for keyvaluemap in "${keyvaluemaps[@]}"; do
  ...
  for entrie in "${entries[@]}"; do
    ...
    json_payload=$(cat "$DEST_DIR/keyvaluemap_${keyvaluemap_name}_entries_${entrie}_details.json")

    # Make a POST request to create the environment in the destination project
    create_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/keyvaluemaps/${keyvaluemap_name}/entries" \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        -H "Content-Type: application/json")

    # Save the response for the created environment to a file
    echo "$create_response" > "$DEST_DIR/keyvaluemap_${keyvaluemap_name}_entries_${entrie}_response.json"
