...
for environment in "${environments[@]}"; do
  ...
  for keyvaluemap in "${keyvaluemap_names[@]}"; do
    ...
    create_response=$(curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/environments/$environment/keyvaluemaps" \
        -H "Authorization: Bearer $DEST_TOKEN" \
        -d "$json_payload" \
        -H "Content-Type: application/json")

    # Save the response for the created environment to a file
    echo "$create_response" > "$DEST_DIR/${environment}_env_${keyvaluemap}_keyvaluemap_response.json"
