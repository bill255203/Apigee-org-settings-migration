...
for email in "${emails[@]}"; do
  ...
  developer_info=$(cat "$DEST_DIR/${email}_developer_details.json")
  
  # Make a request to create or update the developer in the destination project
  curl -X POST "https://apigee.googleapis.com/v1/organizations/$DEST_ORG/developers" -H "Authorization: Bearer $DEST_TOKEN" -o "$DEST_DIR/${email}_developer_response.json" -d "$developer_info" -H "Content-Type: application/json"
