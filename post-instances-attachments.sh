...
for environment in "${environments[@]}"; do
  ...
  for attachment_name in "${attachment_names[@]}"; do
    ...
    json_payload=$(cat "$DEST_DIR/instance_${instance_name}_attachment_details.json")
    ...
    echo "$create_response" > "$DEST_DIR/instance_${instance_name}_attachment_response.json"
  done
done